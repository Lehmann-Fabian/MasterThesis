...
import lehmann.master.thesis.mcc.tu.berlin.de.filter.data.FilteredDataEntryDeserializer;
...
//        this.consumer.subscribe(Collections.singletonList(topic));
	...
	public long getOffset() {
		...
		for (TopicPartition partition : partitions) {
			...
			maxTrials = 10;
			ConsumerRecords<Long, FilteredDataEntry> records;
			...
			for (ConsumerRecord<Long, FilteredDataEntry> record : records) {
				log.info("Got following record: " + record.value().toString() + " for topic: "+ topic);
				return record.value().getOffset();
			}
		}
